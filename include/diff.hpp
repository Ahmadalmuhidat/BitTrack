...
struct DiffLine
{
  ...
  DiffLine(
      DiffLineType diff_type,
      int line_number,
      const std::string &text) : type(diff_type), line_number(line_number), content(text) {}
};
...
struct DiffHunk
{
  ...
  DiffHunk(
      ...
      int new_start,
      int new_count,
      const std::string &header) : old_start(old_string), old_count(old_count), new_start(new_start), new_count(new_count), header(header) {}
};
...
struct DiffResult
{
  ...
  DiffResult(
      ...
      const std::string &file2) : file1(file1), file2(file2), is_binary(false) {}
};
...
bool isBinaryFile(const std::string &file_path); // TODO: move to utils
