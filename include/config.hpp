...
struct ConfigEntry
{
  ...
  ConfigEntry(const std::string& key, const std::string& value, ConfigScope scope): key(key), value(value), scope(scope) {}
};

static std::map<std::string, std::string> global_config;
static std::map<std::string, std::string> repository_config;

void configSet(const std::string& key, const std::string& value, ConfigScope scope = ConfigScope::REPOSITORY);
std::string configGet(const std::string& key, ConfigScope scope = ConfigScope::REPOSITORY);
void configUnset(const std::string& key, ConfigScope scope = ConfigScope::REPOSITORY);
