...
std::vector<std::string> getBranchesList()
{
  ...
  std::vector<std::filesystem::path> branchesPaths = ErrorHandler::safeListDirectoryFiles(".bittrack/refs/heads");
...
void restoreUntrackedFilesFromBackup()
{
  ...
  {
    std::vector<std::filesystem::path> files = ErrorHandler::safeListDirectoryFiles(".bittrack/untracked_backup");

    // Iterate through backup directory to restore untracked files
    for (const auto &entry : files)
    {
      // restore the untracked file to its original location
      std::filesystem::path restore_path = std::filesystem::relative(entry.string(), ".bittrack/untracked_backup");
...
void restoreFilesFromCommit(const std::string &commit_path)
{
  ...
  for (const auto &entry : std::filesystem::directory_iterator(".bittrack/objects"))
  {
    ...
    if (entry.is_directory())
    {
      ...
      std::vector<std::filesystem::path> files = ErrorHandler::safeListDirectoryFiles(commit_dir);

      // Skip if it's the target commit
      for (const auto &file_entry : files)
      {
        // Get the relative path of the file within the commit
        std::filesystem::path relative_path = std::filesystem::relative(file_entry.string(), commit_dir);
  ...
  std::vector<std::filesystem::path> files = ErrorHandler::safeListDirectoryFiles(commit_path);
  for (const auto &entry : files)
  {
    // Get the relative path of the file within the commit
    std::filesystem::path relative_path = std::filesystem::relative(entry.string(), commit_path);
...
void updateWorkingDirectory(const std::string &target_branch)
{
  ...
  if (target_commit.empty())
  {
    ErrorHandler::printError(
        ...
        "No commits found in target branch: " + target_branch,
        ErrorSeverity::ERROR,
  ...
  std::vector<std::filesystem::path> files = ErrorHandler::safeListDirectoryFiles(".");
  for (const auto &entry : files)
  {
...
void addBranch(const std::string &branch_name)
{
  ...
  if (std::find(branches.begin(), branches.end(), branch_name) == branches.end())
  {
    ...
    ErrorHandler::safeWriteFile(".bittrack/refs/heads/" + branch_name, current_commit);
...
void rebaseBranch(const std::string &source_branch, const std::string &target_branch)
{
  try
  {
    ...
    if (current_branch != source_branch)
    {
      ErrorHandler::printError(
          ErrorCode::BRANCH_NOT_FOUND,
          "You must be on the source branch '" + source_branch + "' to rebase",
          ErrorSeverity::ERROR,
          "rebase_branch");
      return;
    }
    ...
    std::string common_ancestor = findCommonAncestor(source_branch, target_branch);
      ...
      for (const auto &commit_hash : commits_to_rebase)
      {
        ...
        if (!applyCommitDuringRebase(commit_hash))
        {
          ErrorHandler::printError(
              ...
              "Failed to apply commit " + commit_hash + " during rebase",
              ErrorSeverity::ERROR,
              "rebase_branch");
          ErrorHandler::printError(
              ErrorCode::COMMIT_FAILED,
...
void cleanupBranchCommits(const std::string &branch_name)
{
  try
  {
    ...
    std::string history_file_content = ErrorHandler::safeReadFile(".bittrack/commits/history");
  ...
  catch (const std::exception &e)
  {
    ErrorHandler::printError(
        ErrorCode::UNEXPECTED_EXCEPTION,
        "Error during commit cleanup: " + std::string(e.what()),
        ErrorSeverity::ERROR,
...
std::vector<std::string> getCommitChain(const std::string &from_commit, const std::string &to_commit)
{
  ...
  catch (const std::exception &e)
  {
    ErrorHandler::printError(
        ErrorCode::UNEXPECTED_EXCEPTION,
        "Error getting commit chain: " + std::string(e.what()),
        ErrorSeverity::ERROR,
        "get_commit_chain");
    return commit_chain;
  }
...
bool applyCommitDuringRebase(const std::string &commit_hash)
{
  try
  {
    ...
    std::string file_content = ErrorHandler::safeReadFile(".bittrack/commits/" + commit_hash);
    ...
    if (new_commit_hash.empty())
    {
      ErrorHandler::printError(
          ErrorCode::COMMIT_FAILED,
          "Failed to create new commit during rebase",
          ErrorSeverity::ERROR,
          "apply_commit_during_rebase");
      return false;
    }
