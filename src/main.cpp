...
void branchFlag(int argc, const char *argv[], int &i)
{
  ...
  else
  {
    throw BitTrackError(
        ...
        "Invalid branch sub-command: " + subFlag + ". Use -l, -c, -r, -m, -i, -h, -merge, or -rebase",
        ErrorSeverity::ERROR,
        "--branch");
  }
}
...
void remoteFlag(int argc, const char *argv[], int &i)
{
  try
  {
    ...
    else if (subFlag == "-d")
    {
      ...
      {
        std::cout << "Deleted remote branch '" << branch_name << "'" << std::endl;
      }
      else
      {
        throw BitTrackError(
    ...
    else
    {
      throw BitTrackError(
          ErrorCode::INVALID_ARGUMENTS,
          "Invalid remote sub-command: " + subFlag + ". Use -v, -s, -l, or -d",
          ErrorSeverity::ERROR,
          "--remote");
    }
  }
