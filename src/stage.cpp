  ...
  if (!is_deleted_file)
  {
    ...
    if (!std::filesystem::exists(actual_path))
    {
      ErrorHandler::printError(
          ...
          "File does not exist: " + actual_path,
...
std::unordered_set<std::string> getTrackedFiles()
{
  ...
  while (std::getline(history_file_content, line))
  {
    ...
    if (iss >> commitHash >> branch && branch == currentBranch)
    {
      ...
      std::vector<std::filesystem::path> commitFiles = ErrorHandler::safeListDirectoryFiles(commitDir);
    ...
    std::unordered_map<std::string, std::string> &staged_files)
{
  ...
  for (const auto &stagedFile : staged_files)
  {
    ...
    if (!isDeleted(stagedFile.first) && !std::filesystem::exists(stagedFile.first))
    {
...
void stage(const std::string &file_path)
{
  try
  {
    ...
    std::unordered_map<std::string, std::string> staged_files = loadStagedFiles();
...
std::vector<std::string> getStagedFiles()
{
  ...
  catch (const std::exception &e)
  {
    ErrorHandler::printError(
        ...
        "Error reading staged files: " + std::string(e.what()),
        ErrorSeverity::ERROR,
        "get_staged_files");
  }
...
std::vector<std::string> getUnstagedFiles()
{
  ...
  catch (const std::exception &e)
  {
    ErrorHandler::printError(
        ErrorCode::UNEXPECTED_EXCEPTION,
        "Error getting unstaged files: " + std::string(e.what()),
        ErrorSeverity::ERROR,
        "get_unstaged_files");
  }
...
std::string getFileHash(const std::string &file_path)
{
  ...
  catch (const std::exception &e)
  {
    ErrorHandler::printError(
        ErrorCode::UNEXPECTED_EXCEPTION,
        "Error getting file hash: " + std::string(e.what()),
        ErrorSeverity::ERROR,
        "get_file_hash");
    return "";
  }
...
bool isDeleted(const std::string &file_path)
{
  ...
  catch (const std::exception &e)
  {
    ErrorHandler::printError(
        ErrorCode::UNEXPECTED_EXCEPTION,
        "Error checking if file is deleted: " + std::string(e.what()),
        ErrorSeverity::ERROR,
        "is_deleted");
    return false;
  }
...
std::string getActualPath(const std::string &file_path)
{
  ...
  catch (const std::exception &e)
  {
    ErrorHandler::printError(
        ErrorCode::UNEXPECTED_EXCEPTION,
        "Error getting actual path: " + std::string(e.what()),
        ErrorSeverity::ERROR,
        "get_actual_path");
    return file_path;
  }
