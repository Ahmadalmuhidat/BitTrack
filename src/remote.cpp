...
void push(const std::string &remote_url_arg, const std::string &branch_name_arg) {
  ...
  if (curl) {
    ...
    curl_formadd(&form, &last, CURLFORM_COPYNAME, "upload", CURLFORM_FILE, ".bittrack/remote_push_folder.zip", CURLFORM_END);

    // Add branch and commit fields
    curl_formadd(&form, &last, CURLFORM_COPYNAME, "branch", CURLFORM_COPYCONTENTS, CurrentBranch.c_str(), CURLFORM_END);

    // Add commit field
    curl_formadd(&form, &last, CURLFORM_COPYNAME, "commit", CURLFORM_COPYCONTENTS, CurrentCommit.c_str(), CURLFORM_END);
...
std::string get_github_commit_for_bittrack(const std::string &bittrack_commit) {
  ...
  while (std::getline(file, line)) {
    ...
    if (iss >> bittrack_hash >> github_hash &&
...
std::vector<std::string> get_committed_files(const std::string &commit) {
  ...
  while (std::getline(file, line)) {
    ...
    if (in_files_section && !line.empty()) // Process lines in the Files section
    {
  ...
  try {
    ...
    if (last_commit_sha.empty()) {
      ...
      if (!local_parent.empty()) {
        ...
        std::string mapped_parent_sha =
            get_github_commit_for_bittrack(local_parent);
    ...
    // std::cout << "Processing commit (" << current_commit.substr(0, 8) << "):
    // " << commit_message << std::endl;
