...
bool pushToGithubApi(const std::string &token, const std::string &username, const std::string &repo_name)
{
  try
  {
    ...
    if (!branch_exists)
    {
      ...
      // main branch.
      std::string main_ref = getGithubLastCommitSha(token, username, repo_name, "heads/main");
      if (main_ref.empty())
      {
    ...
    if (is_empty_repo)
    {
      ...
      std::vector<std::string> latest_commit_files = getCommittedFiles(current_commit);
      ...
      std::string commit_message = getCommitMessage(current_commit); // Get commit message
      ...
      else
      {
        ...
        if (content.empty() && !std::filesystem::exists(commit_file_path))
        {
          ErrorHandler::printError(
              ...
              "Could not read commit file: " + commit_file_path,
              ErrorSeverity::ERROR,
              "push_to_github_api");
          continue;
        }
        ...
        std::string blob_sha = createGithubBlob(token, username, repo_name, content);
        if (!blob_sha.empty())
        {
...
bool pullFromGithubApi(const std::string &token, const std::string &username, const std::string &repo_name, const std::string &branch_name)
{
  try
  {
    ...
    std::string latest_commit_sha = getGithubLastCommitSha(token, username, repo_name, "heads/" + target_branch);
...
bool downloadFilesFromGithubTree(const std::string &token, const std::string &username, const std::string &repo_name, const std::string &tree_data, std::vector<std::string> &downloaded_files)
{
  try
  {
    ...
    for (const auto &file : files)
    {
      ...
      std::string file_content = getGithubBlobContent(token, username, repo_name, file_sha); // Get blob content
...
bool isLocalBehindRemote()
{
  ...
  if (isGithubRemote(remote_url))
  {
    ...
    std::string remote_commit = getGithubLastCommitSha(
