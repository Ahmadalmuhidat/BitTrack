...
bool ErrorHandler::validateFilePath(const std::string &path)
{
  if (path.empty())
  {
    printError(ErrorCode::INVALID_FILE_PATH, "File path cannot be empty", ErrorSeverity::ERROR, "file path validation");
    return false;
  }
...
bool ErrorHandler::validateBranchName(const std::string &name)
{
  if (name.empty())
  {
    printError(ErrorCode::INVALID_BRANCH_NAME, "Branch name cannot be empty", ErrorSeverity::ERROR, "branch name validation");
    return false;
  }
  ...
  {
    printError(ErrorCode::INVALID_BRANCH_NAME, "Branch name contains invalid characters. Use only letters, numbers, dots, underscores, and hyphens", ErrorSeverity::ERROR, name);
    return false;
  }
...
bool ErrorHandler::validateCommitMessage(const std::string &message)
{
  if (message.empty())
  {
    printError(ErrorCode::INVALID_COMMIT_MESSAGE, "Commit message cannot be empty", ErrorSeverity::ERROR, "commit message validation");
    return false;
  }

  if (message.length() > 1000)
  {
    printError(ErrorCode::INVALID_COMMIT_MESSAGE, "Commit message too long (max 1000 characters)", ErrorSeverity::WARNING, "commit message validation");
    return false;
  }
...
bool ErrorHandler::validateRemoteUrl(const std::string &url)
{
  if (url.empty())
  {
    printError(ErrorCode::INVALID_REMOTE_URL, "Remote URL cannot be empty", ErrorSeverity::ERROR, "remote URL validation");
    return false;
  }
  ...
  {
    printError(ErrorCode::INVALID_REMOTE_URL, "Invalid URL format. Use http:// or https:// URLs", ErrorSeverity::ERROR, url);
    return false;
  }
...
bool ErrorHandler::safeCreateDirectories(const std::filesystem::path &path)
{
  ...
  {
    printError(ErrorCode::UNEXPECTED_EXCEPTION, "Unexpected error creating directories: " + std::string(e.what()), ErrorSeverity::FATAL, path.string());
    return false;
  }
}
...
bool ErrorHandler::safeCopyFile(const std::filesystem::path &from, const std::filesystem::path &to)
{
  try
  {
    ...
    {
      printError(ErrorCode::FILE_NOT_FOUND, "Source file does not exist: " + from.string(), ErrorSeverity::ERROR, from.string());
      return false;
    }
  ...
  {
    printError(ErrorCode::UNEXPECTED_EXCEPTION, "Unexpected error copying file: " + std::string(e.what()), ErrorSeverity::FATAL, from.string());
    return false;
  }
}
...
bool ErrorHandler::safeRemoveFile(const std::filesystem::path &path)
{
  ...
  {
    printError(ErrorCode::UNEXPECTED_EXCEPTION, "Unexpected error removing file: " + std::string(e.what()), ErrorSeverity::FATAL, path.string());
    return false;
  }
}
...
bool ErrorHandler::safeWriteFile(const std::filesystem::path &path, const std::string &content)
{
  try
  {
    ...
    if (!file.is_open())
    {
      printError(ErrorCode::FILE_WRITE_ERROR, "Failed to open file for writing: " + path.string(), ErrorSeverity::ERROR, path.string());
      return false;
    }
  ...
  {
    printError(ErrorCode::UNEXPECTED_EXCEPTION, "Unexpected error writing file: " + std::string(e.what()), ErrorSeverity::FATAL, path.string());
    return false;
  }
}
...
std::string ErrorHandler::safeReadFile(const std::filesystem::path &path)
{
  try
  {
    if (!std::filesystem::exists(path))
    {
      printError(ErrorCode::FILE_NOT_FOUND, "File does not exist: " + path.string(), ErrorSeverity::ERROR, path.string());
      return "";
    }
    ...
    if (!file.is_open())
    {
      printError(ErrorCode::FILE_READ_ERROR, "Failed to open file for reading: " + path.string(), ErrorSeverity::ERROR, path.string());
      return "";
    }
  ...
  {
    printError(ErrorCode::UNEXPECTED_EXCEPTION, "Unexpected error reading file: " + std::string(e.what()), ErrorSeverity::FATAL, path.string());
    return "";
  }
}
...
bool ErrorHandler::validateBranchExists(const std::string &branchName)
{
  ...
  {
    printError(ErrorCode::BRANCH_NOT_FOUND, "Branch '" + branchName + "' does not exist", ErrorSeverity::ERROR, branchName);
    return false;
  }
  return true;
}
