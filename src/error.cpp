...
void ErrorHandler::printError(const BitTrackError &error) {
  printError(error.getCode(), error.what(), error.getSeverity(),
             error.getContext());
}

void ErrorHandler::printError(ErrorCode code, const std::string &message,
  ...
  case ErrorCode::NOT_IN_REPOSITORY:
    std::cerr << "  Run 'bittrack init' to initialize a new repository."
              << std::endl;
    break;
  case ErrorCode::INVALID_ARGUMENTS:
    std::cerr << "  Run 'bittrack --help' to see usage information."
              << std::endl;
    break;
  case ErrorCode::BRANCH_NOT_FOUND:
    std::cerr << "  Run 'bittrack --branch -l' to list available branches."
              << std::endl;
    break;
  case ErrorCode::FILE_NOT_FOUND:
    std::cerr << "  Check if the file path is correct and the file exists."
              << std::endl;
    break;
  case ErrorCode::UNCOMMITTED_CHANGES:
    std::cerr << "  Commit or stash your changes before switching branches."
              << std::endl;
    break;
  default:
    break;
...
void ErrorHandler::handleFilesystemError(
    const std::filesystem::filesystem_error &e, const std::string &operation) {
  std::string errorMsg =
      "Filesystem error during " + operation + ": " + e.what();

  if (e.code() == std::errc::no_such_file_or_directory) {
    printError(ErrorCode::FILE_NOT_FOUND, errorMsg, ErrorSeverity::ERROR,
               operation);
  } else if (e.code() == std::errc::permission_denied) {
    printError(ErrorCode::FILE_ACCESS_DENIED, errorMsg, ErrorSeverity::ERROR,
               operation);
  } else if (e.code() == std::errc::no_space_on_device) {
    printError(ErrorCode::INSUFFICIENT_DISK_SPACE, errorMsg,
               ErrorSeverity::FATAL, operation);
  } else {
    printError(ErrorCode::FILE_ACCESS_DENIED, errorMsg, ErrorSeverity::ERROR,
               operation);
  }
...
bool ErrorHandler::validateArguments(int argc, int expected,
...
bool ErrorHandler::validateFilePath(const std::string &path) {
  if (path.empty()) {
    printError(ErrorCode::INVALID_FILE_PATH, "File path cannot be empty",
               ErrorSeverity::ERROR, "file path validation");
    return false;
  }

  if (path.find('\0') != std::string::npos) {
    printError(ErrorCode::INVALID_FILE_PATH,
               "File path contains null character", ErrorSeverity::ERROR, path);
    return false;
  }

  if (path.find("..") != std::string::npos) {
    printError(ErrorCode::INVALID_FILE_PATH,
               "File path contains path traversal", ErrorSeverity::ERROR, path);
    return false;
  }
...
bool ErrorHandler::validateBranchName(const std::string &name) {
  if (name.empty()) {
    printError(ErrorCode::INVALID_BRANCH_NAME, "Branch name cannot be empty",
               ErrorSeverity::ERROR, "branch name validation");
    return false;
  }

  std::regex branchPattern("^[a-zA-Z0-9._-]+$");
  if (!std::regex_match(name, branchPattern)) {
    printError(ErrorCode::INVALID_BRANCH_NAME,
  ...
  if (name == "HEAD") {
    printError(ErrorCode::INVALID_BRANCH_NAME,
               "Branch name '" + name + "' is reserved", ErrorSeverity::ERROR,
               name);
    return false;
  }
...
bool ErrorHandler::validateCommitMessage(const std::string &message) {
  if (message.empty()) {
    printError(ErrorCode::INVALID_COMMIT_MESSAGE,
               "Commit message cannot be empty", ErrorSeverity::ERROR,
  ...
  if (message.length() > 1000) {
    printError(ErrorCode::INVALID_COMMIT_MESSAGE,
               ...
               ErrorSeverity::WARNING, "commit message validation");
    return false;
  }
...
bool ErrorHandler::validateRemoteUrl(const std::string &url) {
  if (url.empty()) {
    printError(ErrorCode::INVALID_REMOTE_URL, "Remote URL cannot be empty",
               ErrorSeverity::ERROR, "remote URL validation");
    return false;
  }

  std::regex urlPattern("^(https?|ftp)://[^\\s/$.?#].[^\\s]*$");
  if (!std::regex_match(url, urlPattern)) {
    printError(ErrorCode::INVALID_REMOTE_URL,
               "Invalid URL format. Use http:// or https:// URLs",
               ErrorSeverity::ERROR, url);
    return false;
  }
...
bool ErrorHandler::safeCreateDirectories(const std::filesystem::path &path) {
  try {
    ...
    printError(ErrorCode::UNEXPECTED_EXCEPTION,
               "Unexpected error creating directories: " +
                   std::string(e.what()),
               ErrorSeverity::FATAL, path.string());
    return false;
  }
}

bool ErrorHandler::safeCopyFile(const std::filesystem::path &from,
                                const std::filesystem::path &to) {
  try {
    if (!std::filesystem::exists(from)) {
      printError(ErrorCode::FILE_NOT_FOUND,
                 "Source file does not exist: " + from.string(),
                 ErrorSeverity::ERROR, from.string());
      return false;
    }
    ...
    printError(ErrorCode::UNEXPECTED_EXCEPTION,
               ...
               ErrorSeverity::FATAL, from.string());
    return false;
  }
}
...
bool ErrorHandler::safeWriteFile(const std::filesystem::path &path,
  ...
  try {
    ...
    if (!file.is_open()) {
      printError(ErrorCode::FILE_WRITE_ERROR,
                 "Failed to open file for writing: " + path.string(),
                 ErrorSeverity::ERROR, path.string());
      return false;
    }
...
bool ErrorHandler::safeAppendFile(const std::filesystem::path &path,
                                  const std::string &content) {
  try {
    if (!path.parent_path().empty()) {
      safeCreateDirectories(path.parent_path());
...
bool ErrorHandler::safeRename(const std::filesystem::path &from,
                              const std::filesystem::path &to) {
  try {
    if (!std::filesystem::exists(from)) {
      printError(ErrorCode::FILE_NOT_FOUND,
                 "Source path does not exist: " + from.string(),
                 ErrorSeverity::ERROR, "rename");
      return false;
    }
...
std::string ErrorHandler::safeReadFile(const std::filesystem::path &path) {
  try {
    if (!std::filesystem::exists(path)) {
      printError(ErrorCode::FILE_NOT_FOUND,
                 "File does not exist: " + path.string(), ErrorSeverity::ERROR,
                 path.string());
      return "";
    }
    ...
    if (!file.is_open()) {
      printError(ErrorCode::FILE_READ_ERROR,
...
ErrorHandler::safeListDirectoryFiles(const std::filesystem::path &path) {
  try {
...
bool ErrorHandler::validateRepository() {
  ...
  if (!std::filesystem::exists(".bittrack/HEAD") ||
      !std::filesystem::exists(".bittrack/commits") ||
      !std::filesystem::exists(".bittrack/refs")) {
    printError(ErrorCode::REPOSITORY_CORRUPTED,
               "Repository appears to be corrupted - missing essential files",
               ErrorSeverity::FATAL, "repository validation");
    return false;
  }
...
bool ErrorHandler::validateBranchExists(const std::string &branchName) {
  ...
  if (!std::filesystem::exists(branchPath)) {
    printError(ErrorCode::BRANCH_NOT_FOUND,
               "Branch '" + branchName + "' does not exist",
               ErrorSeverity::ERROR, branchName);
    return false;
  }
  return true;
}
...
bool ErrorHandler::validateNoUncommittedChanges() {
  try {
    ...
    if (!staged_files.empty()) {
      printError(ErrorCode::VALIDATION_ERROR,
                 "You have staged changes. Please commit or unstage them "
                 "before proceeding.",
                 ErrorSeverity::ERROR, "uncommitted changes");
      return false;
    }
    ...
    printError(ErrorCode::VALIDATION_ERROR,
               "Error checking for uncommitted changes: " +
                   std::string(e.what()),
               ErrorSeverity::ERROR, "validation");
    return false;
  }
}
