...
DiffResult compareFileToContent(const std::string &file,
...
DiffResult diffStaged() {
  ...
  if (current_commit.empty()) {
    ...
    for (const auto &file : staged_files) {
      ...
      if (!content_lines.empty()) {
        // Create hunk
        DiffHunk hunk(0, 0, 1, content_lines.size(),
                      file + ": @@ -0,0 +1," +
                          std::to_string(content_lines.size()) + " @@");
  ...
  for (const auto &file : staged_files) {
    ...
    std::string commit_file =
        ".bittrack/objects/" + current_commit + "/" + file;
    ...
    if (std::filesystem::exists(commit_file)) {
      ...
      if (!content_lines.empty()) {
        // Create hunk
        DiffHunk hunk(0, 0, 1, content_lines.size(),
                      "@@ -0,0 +1," + std::to_string(content_lines.size()) +
                          " @@");
...
DiffResult diffWorkingDirectory() {
  ...
  if (std::filesystem::exists(commit_dir)) {
    ...
    for (const auto &entry : ErrorHandler::safeListDirectoryFiles(commit_dir)) {
      std::string rel_path =
          std::filesystem::relative(entry, commit_dir).string();
  ...
  for (const auto &file : all_files) {
    ...
    if (std::filesystem::exists(commit_file)) {
      ...
      if (!file_lines.empty()) {
        // Create hunk
        DiffHunk hunk(0, 0, 1, file_lines.size(),
                      file + ": @@ -0,0 +1," +
...
std::vector<DiffHunk> computeHunks(const std::vector<std::string> &old_lines,
  ...
  for (const auto &line : diff_lines) {
    ...
    if (line.type == DiffLineType::ADDITION ||
      ...
      if (current_hunk.lines.empty()) {
        ...
        current_hunk.header = "@@ -" + std::to_string(old_line) + ",0 +" +
                              std::to_string(new_line) + ",0 @@";
      }
...
std::vector<DiffLine>
computeDiffLines(const std::vector<std::string> &old_lines,
                 const std::vector<std::string> &new_lines) {
    ...
    if (old_exists && new_exists) {
      ...
      if (old_lines[i] == new_lines[i]) {
        diff_lines.push_back(
            DiffLine(DiffLineType::CONTEXT, i + 1, old_lines[i]));
      } else {
        diff_lines.push_back(
            DiffLine(DiffLineType::DELETION, i + 1, old_lines[i]));
        diff_lines.push_back(
            DiffLine(DiffLineType::ADDITION, i + 1, new_lines[i]));
      }
...
void printDiffLine(const DiffLine &line, const std::string &prefix) {
  std::cout << prefix << getDiffLinePrefix(line.type) << line.content
            << std::endl;
}
